...
def validate_data(dataframe: pd.DataFrame) -> None:
    ...
    if dataframe["currency"].duplicated().any():
        raise ValueError("Duplicate currency were found.")

    if not (dataframe["exchange_rate"] > 0).all():
        raise ValueError("One or more exchange rates are not positive.")

    invalid_codes = dataframe[
        dataframe["currency"].str.len() != 3
    ]
    if not invalid_codes.empty:
        raise ValueError(
            "One or more currency do not contain three characters."
        )
